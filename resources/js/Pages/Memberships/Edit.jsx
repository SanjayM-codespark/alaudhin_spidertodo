...
export default function MembershipsEdit({ membership, organizations, packages }) {
    ...
    const [showTrialFields, setShowTrialFields] = useState(membership.is_trial);
    const [showDiscountFields, setShowDiscountFields] = useState(membership.discount_amount > 0 || membership.coupon_code);
    ...
    const { data, setData, put, processing, errors } = useForm({
        ...
        start_date: formatDateForInput(membership.start_date) || new Date().toISOString().split('T')[0],
        end_date: formatDateForInput(membership.end_date) || '',
        is_trial: membership.is_trial || false,
        trial_ends_at: formatDateForInput(membership.trial_ends_at) || '',
        trial_days: membership.trial_days || 0,
        ...
        paid_at: formatDateForInput(membership.paid_at) || '',
        transaction_id: membership.transaction_id || '',
        payment_details: membership.payment_details || {},
        status: membership.status || 'pending_payment',
        auto_renew: membership.auto_renew || true,
        next_billing_date: formatDateForInput(membership.next_billing_date) || '',
        notes: membership.notes || '',

        // These are read-only but need to be in the form
        allowed_projects: membership.allowed_projects || 0,
        allowed_tasks_per_project: membership.allowed_tasks_per_project || 0,
        allowed_total_tasks: membership.allowed_total_tasks || 0,
        allowed_team_members: membership.allowed_team_members || 1,
        allowed_clients: membership.allowed_clients || 0,
        allowed_storage_mb: membership.allowed_storage_mb || 100,
        feature_snapshot: membership.feature_snapshot || {},
    });
    ...
    useEffect(() => {
        if (data.package_id) {
            ...
            // Update price based on billing cycle if package changed
            if (pkg && pkg.id !== membership.package_id) {
    ...
    const handleBillingCycleChange = (e) => {
        ...
        // Update price if package exists
        if (selectedPackage) {
    ...
    const handleSubmit = (e) => {
        ...
        put(route('memberships.update', membership.id), {
            onSuccess: () => {
                // Success handling is done by Inertia
            },
        });
    };
    ...
    return (
        ...
        >
            <Head title={`Edit Membership - ${membership.organization?.name}`} />
            ...
            <div className={`min-h-screen transition-colors duration-300 ${themeClasses.pageBg}`}>
                <div className="w-full max-w-[1000px] mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-5">
                        ...
                        <div className="relative px-6 sm:px-8 py-7 flex flex-col sm:flex-row sm:items-center justify-between gap-5">
                            <div className="flex-1 min-w-0">
                                ...
                                <h1 className={`text-xl sm:text-2xl font-semibold tracking-tight leading-snug ${themeClasses.welcomeTitle}`}>
                                    Edit <span className={themeClasses.welcomeName}>Membership</span>
                                </h1>
                                <p className={`text-sm font-light mt-2 leading-relaxed max-w-md ${themeClasses.welcomeDesc}`}>
                                    Update subscription details for {membership.organization?.name}.
                                </p>
                            </div>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-5">
                        {/* Organization Selection (Read-only) */}
                        <div className={`relative rounded-2xl border overflow-hidden transition-colors duration-300 ${themeClasses.card}`}>
                        ...
                        <div className={`relative rounded-2xl border overflow-hidden transition-colors duration-300 ${themeClasses.card}`}>
                            ...
                            <div className="px-6 py-5">
                                <h3 className={`text-[13px] font-semibold mb-4 ${themeClasses.heading}`}>Membership Status</h3>
                                ...
                                <div>
                                    ...
                                    {errors.status && (
                                        <p className="mt-1 text-[11px] text-red-400">{errors.status}</p>
                                    )}
                                </div>
                            </div>
                        </div>
                        ...
                        {/* Read-only Limits (from package snapshot) */}
                        <div className={`relative rounded-2xl border overflow-hidden transition-colors duration-300 ${themeClasses.card}`}>
                            <div className="absolute top-0 left-0 right-0 h-px bg-gradient-to-r from-transparent via-violet-500/25 to-transparent" />

                            <div className="px-6 py-5">
                                <h3 className={`text-[13px] font-semibold mb-4 ${themeClasses.heading}`}>Usage Limits (from package snapshot)</h3>
                                ...
                                <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                                    <div>
                                        ...
                                        <p className={`text-[13px] font-semibold ${themeClasses.body}`}>
                                            {membership.allowed_projects === 0 ? 'Unlimited' : membership.allowed_projects}
                                        </p>
                                    </div>
                                    <div>
                                        <p className={`text-[10px] font-medium uppercase tracking-wider mb-1 ${themeClasses.subheading}`}>
                                        ...
                                        <p className={`text-[13px] font-semibold ${themeClasses.body}`}>
                                            {membership.allowed_tasks_per_project === 0 ? 'Unlimited' : membership.allowed_tasks_per_project}
                                        </p>
                                    </div>
                                    <div>
                                        <p className={`text-[10px] font-medium uppercase tracking-wider mb-1 ${themeClasses.subheading}`}>
                                        ...
                                        <p className={`text-[13px] font-semibold ${themeClasses.body}`}>
                                            {membership.allowed_total_tasks === 0 ? 'Unlimited' : membership.allowed_total_tasks}
                                        </p>
                                    </div>
                                    <div>
                                        <p className={`text-[10px] font-medium uppercase tracking-wider mb-1 ${themeClasses.subheading}`}>
                                        ...
                                        <p className={`text-[13px] font-semibold ${themeClasses.body}`}>
                                            {membership.allowed_team_members}
                                        </p>
                                    </div>
                                    <div>
                                        <p className={`text-[10px] font-medium uppercase tracking-wider mb-1 ${themeClasses.subheading}`}>
                                        ...
                                        <p className={`text-[13px] font-semibold ${themeClasses.body}`}>
                                            {membership.allowed_clients === 0 ? 'Unlimited' : membership.allowed_clients}
                                        </p>
                                    </div>
                                    <div>
                                        <p className={`text-[10px] font-medium uppercase tracking-wider mb-1 ${themeClasses.subheading}`}>
                                        ...
                                        <p className={`text-[13px] font-semibold ${themeClasses.body}`}>
                                            {membership.allowed_storage_mb} MB
                                        </p>
                                    </div>
                                </div>
                                <p className={`text-[10px] mt-3 ${themeClasses.muted}`}>
                                    These limits are snapshots from when the membership was created and cannot be edited directly.
                                    To change limits, change the package.
                                </p>
                            </div>
                        </div>
