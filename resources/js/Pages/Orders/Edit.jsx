...
export default function Edit({ order, products }) {
    ...
    const { data, setData, put, processing, errors } = useForm({
        ...
        order_due_date: order.order_due_date ? new Date(order.order_due_date).toISOString().slice(0, 16) : "",
    ...
    // Get due date status for visual indication
    const getDueDateStatus = () => {
        if (!data.order_due_date) return null;

        const dueDate = new Date(data.order_due_date);
                                ...
                                <div className="relative rounded-2xl bg-[#0f1117] border border-white/[0.06] overflow-hidden">
                                    ...
                                    <div className="p-6 space-y-4">
                                        ...
                                        <div className="space-y-2">
                                            ...
                                            <div className="w-full px-4 py-3 bg-white/3 border border-white/5 rounded-xl text-sm text-slate-400 font-mono">
                                                {order.order_number}
                                            </div>
                                        </div>
                                        ...
                                        <div className="space-y-2">
                                            ...
                                            <input
                                                ...
                                                className={`w-full px-4 py-3 bg-white/5 border rounded-xl text-sm text-white/80 focus:outline-none focus:border-amber-400/30 focus:ring-1 focus:ring-amber-400/20 transition-all ${
                                            ...
                                            {data.order_due_date && (
                                                <div className="mt-2 space-y-1">
                                                    <p className="text-xs text-slate-500">
                                                        Expected delivery: <span className={`${
                                                            getDueDateStatus() === 'overdue'
                                                                ? 'text-red-400'
                                                                : getDueDateStatus() === 'due-today'
                                                                ? 'text-amber-400'
